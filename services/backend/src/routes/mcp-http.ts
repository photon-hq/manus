    ...
    try {
      ...
      server.registerTool(
        ...
        async (args: any) => {
          ...
          fastify.log.info({ phoneNumber: targetPhoneNumber, chunks: chunks.length, attachments: body.attachments?.length || 0 }, 'Sending message chunks via MCP-HTTP');
          ...
          if (body.attachments && body.attachments.length > 0) {
            try {
              fastify.log.info({ phoneNumber: targetPhoneNumber, attachmentCount: body.attachments.length }, 'Sending attachments via MCP-HTTP');
          ...
          fastify.log.info({ phoneNumber: targetPhoneNumber, messageGuids }, 'All message chunks sent via HTTP (tracked as 1 DB record)');
